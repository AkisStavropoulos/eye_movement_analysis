function eye_fixation = AnalyseEyefixation(trials,prs)
...
% set up data
for j = 1:length(trials)
    x_fly{j} = trials(j).prs.fireflyposx - trials(j).continuous.xmp;
    y_fly{j} = trials(j).prs.fireflyposy - trials(j).continuous.ymp;
    zle{j} = trials(j).continuous.zle;    yle{j} = trials(j).continuous.yle;
    zre{j} = trials(j).continuous.zre;    yre{j} = trials(j).continuous.yre;
...
TauAdjust = 0;
Ntaus = 0;
delta = trials(1).prs.eye_dist/2;
zt = -prs.height;
saccade_duration = prs.saccade_duration(2);
...
for i=1:ntrls    
    ...
    if any(sac_indx)
        ...
        % keep the biggest saccade (reliable fixating measure)
        t_sac2 = t_sac2(sacampli == max(sacampli));
%         plot([zre{i} yre{i}]);hold off;
...
% start aligned
dt = .2;
nx = -0.1:dt:12.1; % t = 0 is when the target disappears
...
%stop aligned
if TauAdjust
    t_sac_stop = cellfun(@(x,y,z) x - (y-Ntaus*z), t_sac, num2cell(t_stop), num2cell(tau), 'UniformOutput',false);
else
    t_sac_stop = cellfun(@(x,y,z) x - y, t_sac, num2cell(t_stop),'UniformOutput',false);
end
nx = -7.1:dt:0.5;
