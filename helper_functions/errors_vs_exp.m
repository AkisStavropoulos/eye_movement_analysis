function errors_vs_exp(trials,condition,r_tar)

%% plot radial and angular error as a function of trial experience
...
if isempty(condition) % for all trials
    ...
    b = b(1);
    figure;
    subplot(2,1,1);plot(x,y);hline(0);ylim([-300 300]);
    hold on;plot(x,x*b + c,'r','LineWidth',2);
    xlabel('experience [trials]');ylabel('distance error (cm)');title('distance error as a function of trial experience')
    grid on;
    % angular error
    x = [1:length(trials)]';
    X = [x ones(length(x),1)];
    ...
    c = b(2);
    b = b(1);
    subplot(2,1,2);plot(x,y);hline(0);ylim([-30 30]);
    hold on;plot(x,x*b + c,'r','LineWidth',2);
    xlabel('experience [trials]');ylabel('rotational error (deg)');title('angular error as a function of trial experience');
    grid on;
