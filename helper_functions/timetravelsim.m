function [tt,vel,dist] = timetravelsim(a,vmax,ff_dist,prints)
...
minfun  = @(Tpulse)compdist(a,vmax,Tpulse,x);
Tpulse0 = 0.2;
[Tpulse,fval] = fminsearch(minfun,Tpulse0);
...
Tsim = round(Tsim/dt);
j_x = ([ones(1,Tpulse) zeros(1,Tsim-Tpulse)]); % joystick translational input
ts = (1:Tsim)*dt;
...
if a > 0
   ...
   indx = find(v2(Tpulse:end)<=1,1,'first');
   indx = Tpulse + indx;
   tt = indx*dt;
    ...
    if prints
        ...
        plot(ts,v2,'*g');
        vline(tt);
        legend('joystick input','velocity input (unfiltered)','control after filter');
        hold off;
...
elseif a == 0
   ...
   indx = find(v0(Tpulse:end)<=1,1,'first');
   indx = Tpulse + indx;
   tt = indx*dt;
