function [r_tar,r_sub,theta_tar,theta_sub] = scatterDistAng3(trials,tau,fit,plt)
...
for i = 1:length(taulimnames)
    condition{i} = {[trials(1).prs.subject ' - ' '\tau : ' '[' num2str(tau.lims.(taulimnames{i})) ']']};
...
for j = 1:length(taubinnames)
    ...
    r_tar1 = [];
%     colr = ['k' 'r' 'm'];
    for i = 1:length(tau.bins.(taubinnames{j}))
        indx = tau.bins.(taubinnames{j})(i);
    ...
    % save data
    theta_sub.(taubinnames{j}) = theta_sub1;
    theta_tar.(taubinnames{j}) = theta_tar1;
    r_sub.(taubinnames{j}) = r_sub1;
...
if plt
    if fit
        ...
        subplot(1,2,1); hold on;plot(r_tar.(taubinnames{j}),r_sub.(taubinnames{j}),'Color',colr(j,:),'Marker','.','LineStyle','none');
        ...
        subplot(1,2,2); hold on;plot(theta_tar.(taubinnames{j}),theta_sub.(taubinnames{j}),'Color',colr(j,:),'Marker','.','LineStyle','none');
...
% place figures on Center screen in vertical order
if plt
    count = 0;
    for i = 1:length(fig)
