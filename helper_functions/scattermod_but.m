function scattermod_but(trials,stimtype,tau,plt)
...
if plt
    ...
    for j = 1:length(stnames)
        ...
        suptitle([trials(1).prs.subject ' - ' conditionS{j} ' condition ' '- 1st button push']);
    ...
    for i = 1:length(fig)
        ...
        fig(i).Position = [-1.5+(count*(figdim(1)-2)) 1 figdim];  % 50: RL position , 1: UD position
        count = count + 1;
    end
end
