...
[bias,skipsub] = get_bias(subject,params,button_push);
...
colr = repmat(colr(:),1,Nstim/3)'; colr = reshape(colr,[],3);
if Nstim/3 == 3
    taushape = {'o';'o';'o'}; markersize = [3;5;8];
elseif Nstim/3 == 4
    taushape = {'o';'s';'d';'p'};  markersize = [10;10;10;15];
end
taushape = repmat(taushape,Nstim/3,1);
markersize = repmat(markersize,Nstim/3,1);
count1 = 1; count2 = 1;
bias_mu = []; bias_sem = [];
...
for s = 1:size(bias.r,2)
    ...
    errorbar(bias_mu.r(cnd,taugrp),bias_mu.th(cnd,taugrp),bias_sem.r(cnd,taugrp),bias_sem.r(cnd,taugrp),bias_sem.th(cnd,taugrp),bias_sem.th(cnd,taugrp),...
        taushape{s},'Color',colr(s,:),'MarkerSize',markersize(s),'markerfacecolor',colr(s,:),'capsize',0);
