function [epsilon,sd_epsilon] = epsilonerror(r_sub,r_tar)
%% epsilon error
...
if ~isstruct(r_tar)
    ...
    for j = 1:length(condnames)
        ...
        err = r_sub.(condnames{j}) - r_tar.(condnames{j});
        N = length(r_tar.(condnames{j}));
        epsi(j) = (1/N)*sum(err./r_tar.(condnames{j}));
        epsilon.(condnames{j}) = epsi(j);
        sd_epsi(j) = std(err./r_tar.(condnames{j}));
