function errors_vs_vmaxratio(trials,condition)
...
if isempty(condition) % for all trials
    ...
    for n = 1:length(continuous)
        maxv_reached(n) = max(continuous(n).v);
    end
    % radial error
    x = [maxv_reached./[prs.vmax]]';
    X = [x ones(length(x),1)];
    y = ([stats.d_err])';
    [b]=regress(y,X);
    c = b(2);
    ...
    xlabel('maxV reached / Vmax ratio');ylabel('distance error (cm)');title('distance error as a function of maxV reached / Vmax ratio')
    grid on;
    % angular error
    x = [maxv_reached./[prs.vmax]]';
    X = [x ones(length(x),1)];
    ...
    hold on;plot(x,x*b + c,'r','LineWidth',2);
    xlabel('maxV reached / Vmax ratio');ylabel('rotational error (deg)');title('angular error as a function of maxV reached / Vmax ratio');
    grid on;
