...
if any(strcmp(cond,{'stimtype','tau','stimtau'}))
    ...
    elseif strcmp(cond,'tau')
        ...
        for i = 1:length(limnames)
            condnames{i} = ['\tau ' condition{i} ' - [' num2str(subject(1).tau.lims.(limnames{i})(1)) ' - ' num2str(subject(1).tau.lims.(limnames{i})(2)) ']'];
        end
    ...
    elseif strcmp(cond,'stimtau')
        ...
        for s = 1:length(stimnames)
            for i = 1:length(limnames)
                count = count + 1;
                condnames{count} = [stimnames{s} ' and ' '\tau ' condition{count} ' - [' num2str(subject(1).tau.lims.(limnames{i})(1)) ' - ' num2str(subject(1).tau.lims.(limnames{i})(2)) ']'];
            end
        end
    end
