function errors_f_tau(trials,condition,r_tar,r_sub,theta_tar,theta_sub)

%% plot radial and angular error as a function of tau
...
if isempty(condition) % for all trials
    ...
    b = b(1);
    figure;
    subplot(2,1,1);plot(taus,r_sub-r_tar,'.');hline(0);ylim([-300 300]);
    hold on;plot(x,x*b + c,'r');
    xlabel('\tau (s)');ylabel('distance error (cm)');title('distance error as a function of \tau')
    ...
    c = b(2);
    b = b(1);
    subplot(2,1,2);plot(taus,theta_sub-theta_tar,'.');hline(0);ylim([-30 30]);
    hold on;plot(x,x*b + c,'r');
    xlabel('\tau (s)');ylabel('rotational error (deg)');title('angular error as a function of \tau');
    ...
    for j = 1:length(condnames)
        ...
        c = b(2);
        b = b(1);
        subplot(2,1,1);plot(taus, r_sub.(condnames{j}) - r_tar.(condnames{j}),'.');hline(0);ylim([-300 300]);
        hold on;plot(x,x*b + c,'r');
        xlabel('\tau (s)');ylabel('distance error (cm)');title('distance error as a function of \tau')
        ...
        c = b(2);
        b = b(1);
        subplot(2,1,2);plot(taus, theta_sub.(condnames{j}) - theta_tar.(condnames{j}),'.');hline(0);ylim([-30 30]);
        hold on;plot(x,x*b + c,'r');
        xlabel('\tau (s)');ylabel('rotational error (deg)');title('angular error as a function of \tau');
