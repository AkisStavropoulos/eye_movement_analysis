function [mean_err,std_err] = meanbin_signed_error(trials,condition,binsize,plt)
...
if isempty(condition) % for all trials
    ...
    if plt
        ...
        title('mean signed Distance error');grid on;
...
%%
    for j = 1:length(cond)
        ...
        err_sort = [];
        % sort errors based on the distance of the intended target
        [r_tar_sort,indx1] = sort(r_tar(condition.(cond{j})));
        err_sort = d_err(condition.(cond{j})(indx1));
        % find mean and std of bins
        mean_err1 = [];
        std_err1 = [];
        for i = 1:length(tar_bins)-1
    ...
    if plt
        ...
        if any(strcmp(cond{1},'s1')) || any(strcmp(cond{1},'bin1')) % stimtype OR tau bins
            for j = 1:length(cond)
                ...
                xlabel('initial target distance (cm)');ylabel('mean error (cm)');grid on;
                hold on;
            end
