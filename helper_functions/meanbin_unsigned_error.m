function [mean_err,std_err] = meanbin_unsigned_error(trials,r_tar,err,binsize,plt)
...
if ~isstruct(r_tar) % for all trials
...
%     find mean and std of bins
    ...
    if plt
        figure;errorbar(tar_bins(2:end),mean_err,std_err);xlim([0 800]);title('mean unsigned error');
    ...
    if plt
        ...
        if any(strcmp(condnames{1},'s1')) || any(strcmp(condnames{1},'bin1')) % stimtype OR tau bins
            for j = 1:length(condnames)
                ...
                suptitle(trials(1).prs.subject);title(['mean unsigned error, target bins: ' num2str(binsize) ' cm']);
                xlabel('initial target distance (cm)');ylabel('mean error (cm)');
...
%             plot taus across modalities
            ...
            for i = 1:length(Scond)
                ...
                for n = 1:length(indx)
                    ...
                    suptitle([trials(1).prs.subject ' - target bins: ' num2str(binsize) ' cm']);title(['mean unsigned error for ' conditionStim{i}]);
...
%             plot modalities across taus
            ...
            for i = 1:length(JScond)
                ...
                for n = 1:length(indx)
                    errorbar(tar_bins(2:end),mean_err.(condnames{indx(n)}),std_err.(condnames{indx(n)}),'Color',colr(n,:),'Marker','.');
                    xlim([0 800]);ylim([0 350]);
                    suptitle([trials(1).prs.subject ' - target bins: ' num2str(binsize) ' cm']);title(['mean unsigned error for ' conditionJS{i}]);
                    xlabel('initial target distance (cm)');ylabel('mean error (cm)');hold on;
