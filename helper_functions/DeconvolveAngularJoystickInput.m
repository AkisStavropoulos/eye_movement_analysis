...
%% use toeplitz matrix
% DOESN'T WORK PROPERLY FOR ANGULAR VELOCITY, USE SECTION BELOW
% FA*Y = FB*X, X unknown
...
for k = randi(length(trials),1,20)
    ...
    ts = [];
    x = zeros(1,length(trials(k).continuous.w(1:14:end)));
    M = 3; % =length(x) we are looking for, which is equal to length(FB) here
    N = length(FB);

    for i = (M+N-1):length(trials(k).continuous.w(1:14:end))
        y = trials(k).continuous.w(1:14:end);
        y = y(i-((M+N-1)-1):i-2);
...
%         end
    ...
    y = trials(k).continuous.w(1:14:end);
    figure;plot(ts,x);hold on;plot(ts,y,'r'); ylabel('deg/s');
    xlabel('time (s)');title(['trial No. ' num2str(k) ', with toeplitz matrix']);hold off;legend('Joystick input','angular velocity');hold off;
...
for k = randi(length(trials),1,20)
    ...
    y = trials(k).continuous.w(1:14:end); % dt1/dt = 14
    x = [];
    x(1) = 0; x(2) = 0;
    for n = 3:length(y)
    ...
    figure;plot(trials(k).continuous.ts(1:14:end),x);xlabel('time (s)');hold on;ylabel('deg/s');
    plot(trials(k).continuous.ts(1:14:end),y,'r');title(['trial No. ' num2str(k)]);hold off;legend('Joystick input','angular velocity');
end
