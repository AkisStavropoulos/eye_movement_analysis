function [indx] = plot_tau_ranges(mystruct,ind,spec)
% input params struct
...
for i = 1:length(xx)
    ...
    xlabel('T (s)');ylabel('\tau range');grid on;title([spec ': \tau range as a function of T']);ylim([0 max([ubtau{:}])+2]);xlim([min([TT{:}])-1 max([TT{:}])+1]);hold off;
...
%     xlabel('T (s)');ylabel('firefly range');grid on;title('firefly range as a function of T');ylim([0 1100]);xlim([3 15]);hold off;
    ...
    figure(h(2));subplot(3,sbpltcols,i);hold on;errorbar(TT{i},lbtau{i},[],ubtau{i}-lbtau{i},'o');plot(TT{i},tau_range{i},'-o');
    title(['x = ' num2str(xx(i)) ' m']);legend('LB and UB \tau','absolute range');
    xlabel('T (s)');ylabel('\tau range');grid on;suptitle([spec ': \tau range as a function of T']);ylim([0 max([ubtau{:}])+2]);xlim([min([TT{:}])-1 max([TT{:}])+1]);hold off;
