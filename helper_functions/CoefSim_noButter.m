...
% startcheck = input('Do you wanna run the NO BUTTER simulation? 1 for YES, 0 for NO: ');
% if startcheck
...
tau_input =2;
steps = 100;
ff_dist = 300:steps:600;

clear vardist_noBut
prints = 1;
vardist_noBut = RandomFFSim(x,T,tau_input,ff_dist,prints);
% sort based on moog limitations

% no butter
v_input_noBut = {vardist_noBut.traj};
clear GIA;clear GIAerror;
[GIAerror,~,Moog_X_Pos,MaxPos] = getGIAerror(v_input_noBut);
...
plots = 1;
if seeGIA
    thresh = 5; % cm/s^2, transformed in the function
    indx_allow = [];
    [indx_allow,GIAerror] = keeplowGIAerror(GIAerror,thresh,plots);
        bounded_vardist_noBut = vardist_noBut(indx_allow);
end
if seeMOOG
    thresh2 = .02; % m
    [indx_allow2,Moog_X_Pos] = keepawayfromMOOGlimits(Moog_X_Pos,MaxPos,thresh2,plots);
    bounded_vardist_noBut2 = vardist_noBut(indx_allow2);
end
%% analyze ranges of tau, ff distance and vmax
params_noBut = get_ranges(bounded_vardist_noBut,steps);
params_noBut2 = get_ranges(bounded_vardist_noBut2,steps);

ff_range_noBut = get_ff_range_for_taus(bounded_vardist_noBut,steps);
ff_range_noBut2 = get_ff_range_for_taus(bounded_vardist_noBut2,steps);

%% tau plots
ind_noBut = get_params_indices(x,T,params_noBut);
ind_noBut2 = get_params_indices(x,T,params_noBut2);

plot_tau_ranges(params_noBut,ind_noBut,'NO BUTTER');
plot_tau_ranges(params_noBut2,ind_noBut2,'NO BUTTER 2');

%% firefly plots
plot_ff_ranges(ff_range_noBut,'NO BUTTER');
plot_ff_ranges(ff_range_noBut2,'NO BUTTER 2');
